...
const verifyCode = async (req, res) => {
    ...
    if (rows.length == 0) {
        await request(`https://www.fast2sms.com/dev/bulkV2?authorization=UO2FVJzFRHThMIPKOxZQuH66Habj0gzZnId8b04yJpOW2nyxIUFxwcMOyjao&variables_values=${otp}&route=otp&numbers=${phone}`, async (error, response, body) => {
        ...
        let user = rows[0];
        if (user.time_otp - now <= 0) {
            request(`https://www.fast2sms.com/dev/bulkV2?authorization=UO2FVJzFRHThMIPKOxZQuH66Habj0gzZnId8b04yJpOW2nyxIUFxwcMOyjao&variables_values=${otp}&route=otp&numbers=${phone}`, async (error, response, body) => {
                let data = JSON.parse(body);
                if (data.message == 'SMS sent successfully.') {
